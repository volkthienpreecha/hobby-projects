...
app.post('/api/analyze', async (req, res) => {
  try {
    ...
    if (error.name === 'ClaudeError' || error.response?.status === 429) {
      ...
      return res.status(400).json({ 
        status: 'error',
        message: 'Bad request to Claude API. Terms may contain prohibited content.',
        code: 'BAD_REQUEST'
      });
...
**IMPORTANT: You must respond with valid JSON only. Do not include any text before or after the JSON object. Your entire response should be parseable as JSON.**`;
